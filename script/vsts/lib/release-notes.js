...
module.exports.generateForNightly = async function(
  ...
  const output = [
    `### This nightly release is based on https://github.com/atom-editor/atom/commit/${latestCommit} :atom: :night_with_stars:`
  ];
    ...
    if (oldReleaseNotes) {
      ...
      if (extractMatch.length > 1 && extractMatch[1]) {
        ...
        if (
          ...
          output.push(
            `Click [here](https://github.com/atom-editor/atom/compare/${previousCommit}...${latestCommit}) to see the changes included with this release!`
          );
        }
      }
    }
